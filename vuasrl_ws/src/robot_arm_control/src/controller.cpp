#include "robot_arm_control/robot_arm_keyboard.h"

RobotArmTeleop::RobotArmTeleop()
: node_handle_(""),
  priv_node_handle_("~")
{
  /************************************************************
  ...
  ************************************************************/
  ...
  ROS_INFO("RobotArmTeleop teleoperation using keyboard start");
}

RobotArmTeleop::~RobotArmTeleop()
{
  restoreTerminalSettings();
  ROS_INFO("Terminate RobotArmTeleop Joystick");
  ros::shutdown();
}

void RobotArmTeleop::initClient()
{
  goal_joint_space_path_client_ = node_handle_.serviceClient<robot_arm_msgs::SetJointPosition>("goal_joint_space_path");
  goal_joint_space_path_from_present_client_ = node_handle_.serviceClient<robot_arm_msgs::SetJointPosition>("goal_joint_space_path_from_present");
  goal_task_space_path_from_present_position_only_client_ = node_handle_.serviceClient<robot_arm_msgs::SetKinematicsPose>("goal_task_space_path_from_present_position_only");
  goal_tool_control_client_ = node_handle_.serviceClient<robot_arm_msgs::SetJointPosition>("goal_tool_control");
}

void RobotArmTeleop::initSubscriber()
{
  joint_states_sub_ = node_handle_.subscribe("joint_states", 10, &RobotArmTeleop::jointStatesCallback, this);
  kinematics_pose_sub_ = node_handle_.subscribe("kinematics_pose", 10, &RobotArmTeleop::kinematicsPoseCallback, this);
}

void RobotArmTeleop::jointStatesCallback(const sensor_msgs::JointState::ConstPtr &msg)
{
...
void RobotArmTeleop::kinematicsPoseCallback(const robot_arm_msgs::KinematicsPose::ConstPtr &msg)
{
...
std::vector<double> RobotArmTeleop::getPresentJointAngle()
{
  return present_joint_angle_;
}

std::vector<double> RobotArmTeleop::getPresentKinematicsPose()
{
  return present_kinematic_position_;
}

bool RobotArmTeleop::setJointSpacePathFromPresent(std::vector<std::string> joint_name, std::vector<double> joint_angle, double path_time)
{
  robot_arm_msgs::SetJointPosition srv;
...
bool RobotArmTeleop::setJointSpacePath(std::vector<std::string> joint_name, std::vector<double> joint_angle, double path_time)
{
  robot_arm_msgs::SetJointPosition srv;
  srv.request.joint_position.joint_name = joint_name;
  srv.request.joint_position.position = joint_angle;
...
bool RobotArmTeleop::setToolControl(std::vector<double> joint_angle)
{
  robot_arm_msgs::SetJointPosition srv;
...
bool RobotArmTeleop::setTaskSpacePathFromPresentPositionOnly(std::vector<double> kinematics_pose, double path_time)
{
  robot_arm_msgs::SetKinematicsPose srv;
...
void RobotArmTeleop::printText()
{
  printf("\n");
  printf("---------------------------\n");
  printf("Control Your RobotArm!\n");
  printf("---------------------------\n");
...
void RobotArmTeleop::setGoal(char ch)
{
...
void RobotArmTeleop::restoreTerminalSettings(void)
{
  tcsetattr(0, TCSANOW, &oldt_);  /* Apply saved settings */
}

void RobotArmTeleop::disableWaitingForEnter(void)
{
  struct termios newt;
...
{
  // Init ROS node
  ros::init(argc, argv, "RobotArmTeleop_keyboard");
  RobotArmTeleop RobotArmTeleop;

  char ch;
  RobotArmTeleop.printText();
  while (ros::ok() && (ch = std::getchar()) != 'q')
  {
    ros::spinOnce();
    RobotArmTeleop.printText();
    ros::spinOnce();
    RobotArmTeleop.setGoal(ch);
  }
